...
def getcase_ij(img,coord,i=0,j=0):
    # ligne i, colonne j
    x= coord[0,:]
    y= coord[1,:]
    case = img[y[2*j]:y[2*j+1],x[2*i]:x[2*i+1]]
...
def getcases(img,coord,taille_case=30):
    grid=[]
    for i in range(9):
        for j in range(9):
            grid.append(
                
                cv2.resize (getcase_ij(img,coord,i,j), dsize=(taille_case,taille_case) ) ) 
            
    grid=np.array(grid)
    #breakpoint()
    grid=np.reshape(grid,(9,9,30,30))
